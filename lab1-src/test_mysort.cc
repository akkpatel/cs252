...
testWithInt()
{
  ...
  int testing = sizeof(test)/sizeof(int);
  printf("--- Numbers ----\n");
  ...
  mysort( testing, sizeof(int), test, 1, compareInt );
  ...
  //test that testing numbers are ordered.
  for ( z = 0; z < nelements -1 ; z++ ) {
    assert( a[z] <= a[z+1] );
  }
...
testWithStudents()
{
  ...
  printf("----- Students Sorted by Grade in Descending order.(Trivial test)----\n");
	 mysort( nelements, sizeof(Student), students, 0, compareStudentByGrade);
